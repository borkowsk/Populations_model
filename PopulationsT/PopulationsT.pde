...
boolean WRITENETS=false; //Zapisywanie plików stanu sieci na początku i co 1000 kroków.

anArea island=new anArea(); //Pojemnik na zbiór populacji - na razie pojedynczy

String modelName="PopT";
String lastDescr;
...
void draw()
{
  ...
  if(StepCounter==0)
  {
      ...
      if(GENERATEMOVIE)
            FirstVideoFrame();
      if(WRITENETS) write(island,modelName+".0START"); //Startowy stan ekosystemu
      fill(5);
